...
const PrivacyModal: React.FC<PrivacyModalProps> = ({ onClose }) => {
    return (
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50 animate-[fadeIn_0.2s_ease-out]" onClick={onClose}>
            <div className="bg-white rounded-3xl max-w-lg w-full p-8 shadow-2xl animate-[slideUp_0.3s_ease-out]" onClick={(e) => e.stopPropagation()}>
                <div className="flex items-center gap-3 mb-6">
                    <div className="w-12 h-12 bg-gradient-to-br from-teal-500 to-teal-400 rounded-full flex items-center justify-center text-2xl">
                        <span>🔒</span>
                    </div>
                    <h2 className="text-gray-800 text-2xl font-bold m-0">Política de Privacidad</h2>
                </div>
                ...
                <button
                    onClick={onClose}
                    className="w-full bg-gradient-to-br from-teal-500 to-teal-400 text-white border-none p-3.5 rounded-xl text-base font-semibold cursor-pointer mt-6 transition-all duration-200 shadow-sm hover:-translate-y-0.5 hover:shadow-md"
                >
                    Entendido
                </button>
            </div>
